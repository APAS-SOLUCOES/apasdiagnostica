...
export function getAdaptiveNarrative(scores: ScoreResult): CombinationNarrative {
  ...
  if (!base) throw new Error("Narrativa DISC padrão não encontrada");
  const p = scores.predominant;
  const s = scores.secondary;
  const pv = scores.adapted.percent[p];
  const sv = scores.adapted.percent[s];
  ...
  const experiments = [
    base.experiments[0] ?? `Observe uma situação em que ${DIMENSION_CONTENT[p].title.toLowerCase()} aparece primeiro.`,
    intensityExperiment,
...
function combinationImpact(combination: string, primaryValue: number, gap: number) {
  ...
  if (!options) throw new Error("Impacto DISC padrão não encontrado");
  const index = gap <= 3 ? 0 : gap <= 8 ? 1 : 2;
  const selected = options[index] ?? options[0];
  if (!selected) throw new Error("Impacto DISC não encontrado");
  return {
...
function impactIdentity(d: Dimension, value: number) {
  ...
  if (!firstChoice) return DIMENSION_CONTENT[d].title;
  let identity = firstChoice[1];
