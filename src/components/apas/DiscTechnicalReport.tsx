...
function TechnicalPage({ number, title, kicker, children, dark = false }: { number: number; title: string; kicker: string; children: ReactNode; dark?: boolean }) {
  return <section className={`disc-tech-page ${dark ? "disc-tech-page-dark" : ""}`}><header><span className="disc-tech-brand"><strong>APAS</strong> SOLUÇÕES</span><span>Relatório técnico · Gestão de pessoas</span></header><main><p className="disc-tech-kicker">{kicker}</p><h1>{title}</h1>{children}</main><footer><span>Uso confidencial · Especialista autorizado</span><span>{String(number).padStart(2, "0")}</span></footer></section>;
}
...
export function DiscTechnicalReport({ assessment, scores, computedAt }: { assessment: TechnicalAssessment; scores: ScoreResult; computedAt?: string | null }) {
  ...
  return <article className="disc-technical-report" aria-label={`Relatório técnico de ${assessment.candidate_name}`}>
    ...
    <TechnicalPage number={8} kicker="Plano de ação gerencial" title="Transforme a leitura em uma solução prática">
      ...
      <p className="disc-tech-confidential">Gerado em {computedAt ? new Date(computedAt).toLocaleString("pt-BR") : "data não disponível"} · ID {assessment.id}</p>
    </TechnicalPage>
  </article>;
}