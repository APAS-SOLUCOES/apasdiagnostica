...
export function DiscTechnicalPanel({ assessment, scores, computedAt }: { assessment: TechnicalAssessment; scores: ScoreResult; computedAt?: string | null }) {
  const alerts = [
  ...
  return <div className="space-y-6 print:hidden">
    ...
    <section className="rounded-lg border border-border bg-card p-5"><p className="eyebrow">Indicadores técnicos</p><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-border"><th className="py-2">Fator</th><th>MAIS</th><th>MENOS</th><th>Saldo</th><th>Natural</th><th>Social</th><th>Adaptado</th></tr></thead><tbody>{DIMENSIONS.map((d) => <tr key={d} className="border-b border-border/60"><td className="py-2 font-medium">{d} · {DIMENSION_NAMES[d]}</td><td>{scores.counts?.most[d] ?? "—"}</td><td>{scores.counts?.least[d] ?? "—"}</td><td>{scores.net?.[d] ?? "—"}</td><td>{scores.natural.percent[d]}%</td><td>{scores.social.percent[d]}%</td><td>{scores.adapted.percent[d]}%</td></tr>)}</tbody></table></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><p><span className="text-muted-foreground">Índice de adaptação</span><br />{scores.adaptationIndex}</p><p><span className="text-muted-foreground">Consistência da aplicação</span><br />{scores.answeredItems === scores.totalItems ? "Cobertura integral" : `${scores.answeredItems} de ${scores.totalItems} blocos`}</p><p><span className="text-muted-foreground">Combinação observada</span><br />{scores.combination}</p></div><p className="mt-4 text-xs text-muted-foreground">Método preservado: MAIS/MENOS e parâmetros da versão vinculada à aplicação. Os três perfis, os níveis e o índice exibidos são os valores salvos pelo motor atual, sem recálculo na interface.</p></section>
