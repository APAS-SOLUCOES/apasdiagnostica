...
function Page({ number, title, eyebrow, icon: Icon, children, cover = false, dark = [3, 6, 8, 9, 12].includes(number) }: { number: number; title?: string; eyebrow?: string; icon?: LucideIcon; children: ReactNode; cover?: boolean; dark?: boolean }) {
...
export function DiscPremiumReport({ assessment, scores }: { assessment: ReportAssessment; scores: ScoreResult }) {
  ...
  return <article className="disc-report" aria-label={`Relatório DISC de ${assessment.candidate_name}`}>
    ...
    <Page number={4} eyebrow="03. Seu jeito de agir" title="Seu jeito de agir" icon={Focus}><div className="disc-combination"><span>{scores.combination}</span><div><small>{scores.combinationLabel ?? `${scores.predominant} primário · ${scores.secondary} secundário`}</small><h3>{narrative.title}</h3><p>{narrative.essence}</p></div></div><div className="disc-impact-intro"><span>{primaryImpact.identity}</span><h4>{primaryImpact.phrase}</h4></div><p className="disc-opening">{narrative.profilePortrait}</p><div className="disc-factor-grid">{DIMENSIONS.map((d) => <FactorMark key={d} dimension={d} percent={scores.adapted.percent[d]} />)}</div><SignalCard type="observe" title="Uma hipótese para validar"><p>Em quais situações essa combinação aparece de forma produtiva? E em quais situações você precisa acessar comportamentos diferentes?</p></SignalCard></Page>

    <Page number={5} eyebrow="04. Seus pontos fortes" title="Seus pontos fortes" icon={Sprout}><p className="disc-opening">Quando contexto, clareza e energia se alinham, estas são contribuições que podem aparecer com mais naturalidade.</p><p className="disc-highlight">{narrative.situations?.work}</p><SignalList type="strength" title="Recursos da sua combinação" items={narrative.best}/><div className="disc-two-columns"><SignalList type="strength" title={`Contribuição de ${scores.predominant}`} items={primary.strengths.slice(0, 3)}/><SignalList type="tip" title={`Apoio de ${scores.secondary}`} items={secondary.strengths.slice(0, 3)}/></div><div className="disc-editorial-quote">Força não é um elogio automático: é um recurso que produz valor quando encontra intenção, contexto e medida.</div></Page>
    ...
    <Page number={8} eyebrow="07. Comunicação" title="Comunicação" icon={MessageCircle}><div className="disc-signal-grid"><SignalCard type="strength" title="Quando está no seu melhor"><p>{narrative.communication}</p></SignalCard><SignalCard type="observe" title="Vale observar"><p>Se a mensagem foi compreendida, se houve espaço real para resposta e se os acordos ficaram claros.</p></SignalCard><SignalCard type="attention" title="Ponto de atenção"><p>{primary.attention[0]}.</p></SignalCard><SignalCard type="tip" title="Experimente"><p>{primary.development[0]}. Ao final, confirme quem fará o quê e até quando.</p></SignalCard></div><div className="disc-editorial-quote">Comunicar não é apenas expressar intenção. É também observar o efeito e ajustar a forma sem perder autenticidade.</div></Page>

    <Page number={9} eyebrow="08. Decisão" title="Decisão" icon={Network}><ThemeBlock icon={Focus} title="Como você tende a decidir">{narrative.situations?.decisions ?? narrative.decision}</ThemeBlock><div className="disc-two-columns"><SignalCard type="strength" title="Quando está no seu melhor"><p>{primary.strengths[0]}. {secondary.strengths[0]}.</p></SignalCard><SignalCard type="observe" title="Vale observar"><p>Em decisões relevantes, dê espaço para o fator complementar antes de concluir. Isso amplia a qualidade da leitura sem exigir que você abandone seu estilo.</p></SignalCard></div><SignalCard type="tip" title="Dica"><p>Antes de decidir, faça uma pausa curta e pergunte: “O que ainda preciso considerar antes de escolher?”</p></SignalCard></Page>

    <Page number={10} eyebrow="09. Relacionamentos e equipe" title="Relacionamentos e equipe" icon={Users}><ThemeBlock icon={Compass} title="Liderança">{narrative.situations?.leadership ?? narrative.leadership}</ThemeBlock><ThemeBlock icon={Users} title="Equipe e colaboração">{narrative.team}</ThemeBlock><SignalCard type="observe" title="Vale observar"><p>{narrative.situations?.relationships ?? narrative.perceived}</p></SignalCard><SignalCard type="tip" title="Lembre-se"><p>Grandes resultados são construídos com pessoas, não apenas com esforço individual.</p></SignalCard></Page>
