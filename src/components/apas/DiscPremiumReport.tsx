...
function Page({ number, title, eyebrow, icon: Icon, children, cover = false, dark = false }: { number: number; title?: string; eyebrow?: string; icon?: LucideIcon; children: ReactNode; cover?: boolean; dark?: boolean }) {
...
function DiscOverview({ values }: { values: DimensionMap }) {
  return <div className="disc-overview" aria-label="Visão central das intensidades D I S C"><div className="disc-overview-core"><span>DISC</span><small>visão integrada</small></div>{DIMENSIONS.map((d) => <div className={`disc-overview-ray disc-overview-${d.toLowerCase()}`} key={d}><span className={FACTOR_CLASS[d]} style={{ width: `${Math.max(14, values[d])}%` }} /><strong>{d}</strong><small>{values[d]}%</small></div>)}</div>;
}
...
export function DiscPremiumReport({ assessment, scores }: { assessment: ReportAssessment; scores: ScoreResult }) {
  ...
  return <article className="disc-report" aria-label={`Relatório DISC de ${assessment.candidate_name}`}>
    ...
    <Page number={3} eyebrow="Seu perfil em números" title="Três perspectivas do mesmo repertório" icon={Gauge}><div className="disc-profile-visual"><DiscOverview values={scores.adapted.percent}/><div className="disc-profile-grid"><ProfileBars title="Natural" subtitle="Como tende a agir quando está mais à vontade." values={scores.natural.percent}/><ProfileBars title="Adaptado" subtitle="Como ajusta o comportamento às exigências percebidas." values={scores.adapted.percent}/><ProfileBars title="Social" subtitle="Como acredita que precisa se apresentar no ambiente." values={scores.social.percent}/></div></div><ProfileTable vectors={[{ label: "Natural", value: scores.natural }, { label: "Adaptado", value: scores.adapted }, { label: "Social", value: scores.social }]}/><div className="disc-index"><span>Índice de adaptação</span><strong>{scores.adaptationIndex}</strong><p>{adaptationText}</p></div><p className="disc-note">Nenhuma perspectiva é melhor. Juntas, elas ajudam a compreender repertório e contexto. Gráfico e tabela usam exatamente os mesmos percentuais calculados para esta aplicação.</p></Page>
